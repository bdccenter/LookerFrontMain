...
const mapearClienteCSVaCliente = (clienteCSV: ClienteCSV, index: number): Cliente => {
  ...
  if (campoFecha) { // Si existe el campo de fecha 
    try { // Intentar convertir la fecha a objeto DATE 
      ...
      if (typeof fechaStr === 'string' && (fechaStr.includes('value') || fechaStr.startsWith('{'))) {
        try { // intentar parsear como JSON 
          const fechaObj = JSON.parse(fechaStr);
          if (fechaObj.value) { // Si tiene la propiedad 'value', usarla
            fechaStr = fechaObj.value;
          }
        } catch (e) { // Si no es un JSON válido, lo dejamos como está
      ...
      if (typeof fechaStr === 'string') { // Asegurarnos de que sea un string
        if (fechaStr.includes('-')) { // Corroborar el formato Date YYYY-MM-DD
          // Formato YYYY-MM-DD
    ...
    } catch (error) { // Si hay un error al convertir la feecha, lo manejamos
  ...
  if (ultimaVisita && isNaN(ultimaVisita.getTime())) { // verificar si es una fecha valida
    ultimaVisita = undefined; // Si noes valida, estavlecerla a undefined
  }
...
const cargarDatosCSVCompleto = async (intentos = 3): Promise<void> => {
  ...
  try {
    ...
    if (!archivoActual) { // Si no hay archivo actual, usar el de Gran Auto por defecto
      throw new Error('No se ha seleccionado un archivo CSV para cargar');
    }

    console.log(`Cargando archivo CSV: ${archivoActual}`); // Log para depuración

    // Obtener la configuración de la agencia actual
    const agenciaActual = Object.entries(configuracionAgencias)
      .find(([_, config]) => config.archivo === archivoActual);

    if (!agenciaActual) { // Si no se encuentra la agencia, lanzar error
      throw new Error(`No se encontró configuración para el archivo ${archivoActual}`);
    }

    const [nombreAgencia, config] = agenciaActual; // Desestructuración para obtener el nombre y la configuración
    ...
    try { // Intentar cargar la versión comprimida
      ...
      if (compressedResponse.ok) { // Si la respuesta es correcta, procesar el archivo comprimido
        console.log("Versión comprimida encontrada, descomprimiendo...");
        // Es un archivo comprimido
        const compressedData = await compressedResponse.arrayBuffer(); // Obtener el ArrayBuffer
        const decompressedData = pako.inflate(new Uint8Array(compressedData), { to: 'string' }); // Descomprimir el ArrayBuffer
        csvTextCache = decompressedData; // Almacenar el texto descomprimido en caché
        console.log("Archivo descomprimido correctamente");
      } else { // Si no se encuentra la versión comprimida, intentar cargar el archivo normal 
      ...
      if (!response.ok) { //  si la respuesta no es correcta, lanzara un error
        throw new Error(`Error al cargar el archivo ${archivoActual}: ${response.status} - ${response.statusText}`);
      }
      
      // Almacenar el texto del CSV en cache
      csvTextCache = await response.text();
    }

    // Verificar que el contenido sea realmente un CSV
    if (csvTextCache.trim().startsWith('<!doctype html>') || csvTextCache.trim().startsWith('<html>')) { // Verificar si es HTML
      // Si el contenido comienza con <!doctype html> o <html>, es probable que sea un error de carga
      // o una página de error en lugar de un CSV
      console.error('El contenido devuelto es HTML, no un CSV');
      throw new Error(`El contenido devuelto es HTML, no un CSV. Archivo: ${archivoActual}`); // Lanzar error
    }

    // Verificar que haya contenido real
    if (csvTextCache.trim().length < 100) { // Verificar si el CSV esta vacio o corrupto
      // Si el contenido es muy corto, probablemente esté vacío o corrupto 
      // o no es un CSV válido
    ...
    const resultado = await new Promise<Papa.ParseResult<ClienteCSV>>((resolve, reject) => {
      Papa.parse<ClienteCSV>(csvTextCache!, {
        ...
        complete: (result) => {
          if (result.errors && result.errors.length > 0) { // Verificar si hay errores de parseo ayudando a depurar
